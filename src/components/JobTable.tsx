        ...
        <div className='w-full'>
            ...
            <table>
                <thead className='flex justify-start bg-blue-700 text-white p-2 rounded'>
                    ...
                    {
                        user?.role === "employer" && <th className='w-44 py-2 text-center'>Applicants</th>
                    }
                ...
                <tbody>
                    {
                        jobs && jobs.map((job: {}, index: number) => {
                            const {_id, title, location, jobType, salary, applicants}: any = job;
                            return (
                                <tr key={index} className='flex justify-start p-2 border hover:rounded hover:text-white hover:bg-blue-500'>
                                    ...
                                    <td className='w-44 text-center py-2'>{salary}</td>
                                    {
                                        ...
                                        className="bg-yellow-700 text-white rounded"
                                        >
                                        <td className='w-44 text-center py-2'>{applicants.length} See All</td>
                                        </button>
                                    }
